...
options: {
	...
	secondMeasureType: {
     ...
     section: 'Format',
     values: [{'Independent':'ind'},{'Margin/Average of First Measure':'mof'},{'Margin/Average of Third Measure':'mot'},{'Variance of First Measure':'vof'},{'Variance of Third Measure':'vot'},{'Simple Average of Second Measure':'simpleAverage'}],
     default: 'mof',
     order: 5
    },
...
$(element).append(legendElement);
	...
	update : function (data, element, config, queryResponse){
		...
		var vFirstMeasureFormatDollar = '';
		if( config.firstMeasureFormatName == 'number' || config.firstMeasureFormatName == 'average'){
			vFirstMeasureFormatDollar = data[0][config.query_fields["dimensions"][3].name].value;
		}
		...
		var vSecondMeasureFormatDollar = '';
		if( config.secondMeasureFormatName == 'number' || config.secondMeasureFormatName == 'average'){
			vSecondMeasureFormatDollar = data[0][config.query_fields["dimensions"][3].name].value;
		}
		if( config.secondMeasureType == 'simpleAverage' ){
			vSecondMeasureFormatDollar = '';
		}
		...
		function main(o, data) {
			...
			function appendsecondmeasurevalues(d) {
				...
				// Display $ Avg/Unit accumilated value at primary dimension level
					...
					//Simple average of Second measure.
					else if(vSecondMeasureType == 'simpleAverage'){
						valueinpcnt = d.y2value / d.y3value;
					}
			...
			function name(d) {
				return d.parent
					? name(d.parent) + " / " + d.key + " ("+ vFirstMeasureFormatDollar + formatFirstM(d.value) + ")"
					: d.key + " ("+ vFirstMeasureFormatDollar + formatFirstM(d.value) + ")";
			}
